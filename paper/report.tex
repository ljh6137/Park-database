...
并由林杰泓负责用户信息管理及用户租赁管理的前端和后端开发和设计，由刘艺凡负责车辆信息管理，车辆租赁信息管理和并发管理控制的前端和后端的设计和开发。系统交互页面的设计及优化由两人共同完成。同时，我们共同负责了系统的安全性设计，包括对用户，车辆信息的保护，以及对用户，管理员的权限设置及授权。
...
\begin{figure}[htbp]  % figure 环境用于插入图片并进行浮动
    ...
    \caption{并发访问冲突示例}  % 图片标题
    \label{fig:exam}  % 图片标签，便于引用
\end{figure}
